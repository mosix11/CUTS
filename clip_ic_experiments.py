...
def apply_tv(outputs_dir: Path, results_dir: Path, cfg: dict, cfg_name:str):
    ...
    if 'alpha_IC' not in results_dict:
    ...
    if 'Random Vector' not in results_dict:
        model.load_state_dict(mix_weights, strict=False)
        alpha_IC = results_dict['alpha_IC']
        task_vectors['Random Vector'].apply_to(model, scaling_coef=alpha_IC, strict=False)
    ...
    tmp_dataset.inject_noise(**noise_tv)
    # Exclude clean samples from target class
    clean_ho_ds, noisy_ho_ds = tmp_dataset.get_clean_noisy_subsets('Heldout')
    tmp_dataset.switch_labels_to_clean(noisy_ho_ds)
    tmp_dataset.set_heldoutset(noisy_ho_ds)
