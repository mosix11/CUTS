...
class CNN5_GN(nn.Module):
    ...
    def __init__(
        ...
        self.net = nn.Sequential(
            nn.Conv2d(3, num_channels, kernel_size=3, stride=1, padding=1, bias=True),
            nn.GroupNorm(8, num_channels),
            nn.ReLU(),

            nn.Conv2d(num_channels, num_channels*2, kernel_size=3, stride=1, padding=1, bias=True),
            nn.GroupNorm(8, num_channels*2),
            nn.ReLU(),
            nn.MaxPool2d(2),

            nn.Conv2d(num_channels*2, num_channels*4, kernel_size=3, stride=1, padding=1, bias=True),
            nn.GroupNorm(8, num_channels*4),
            nn.ReLU(),
            nn.MaxPool2d(2),

            nn.Conv2d(num_channels*4, num_channels*8, kernel_size=3, stride=1, padding=1, bias=True),
            nn.GroupNorm(8, num_channels*8),
            nn.ReLU(),
            nn.MaxPool2d(2),

            nn.MaxPool2d(4),
            Flatten(),
            nn.Linear(num_channels*8, num_classes)
        )
