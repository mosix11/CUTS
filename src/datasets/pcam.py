...
class PCAM(BaseClassificationDataset):
    ...
    def load_train_set(self):
        ...
        return datasets.PCAM(root=self.dataset_dir, split="train", transform=self.train_transforms, download=True)
    
    def load_validation_set(self):
        self.val_transforms = self.get_transforms(train=False)
        return datasets.PCAM(root=self.dataset_dir, split="val", transform=self.val_transforms, download=True)
    
    def load_test_set(self):
        self.val_transforms = self.get_transforms(train=False)
        return datasets.PCAM(root=self.dataset_dir, split="test", transform=self.val_transforms, download=True)
