...
class CIFAR100:
    def __init__(
        ...
        if self.class_subset:
            ...
            self.available_classes = list(range(100)) # All CIFAR-100 classes
    ...
    def _init_loaders(self):
        train_dataset = datasets.CIFAR100(root=self.dataset_dir, train=True, transform=self.get_transforms(train=True), download=True)
        test_dataset = datasets.CIFAR100(root=self.dataset_dir, train=False, transform=self.get_transforms(train=False), download=True)
