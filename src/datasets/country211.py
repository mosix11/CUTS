...
class Country211(BaseClassificationDataset):
    ...
    def load_train_set(self):
        ...
        trainset = datasets.Country211(root=self.dataset_dir, split="train", transform=self.train_transforms, download=True)
        self._class_names = trainset.classes
        return trainset
    
    def load_validation_set(self):
        self.val_transforms = self.get_transforms(train=False)
        return datasets.Country211(root=self.dataset_dir, split="valid", transform=self.val_transforms, download=True)
    
    def load_test_set(self):
        self.val_transforms = self.get_transforms(train=False)
        return datasets.Country211(root=self.dataset_dir, split="test", transform=self.val_transforms, download=True)
