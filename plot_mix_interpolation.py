...
#     - No color dependence: styles work in grayscale
#     - Test:   solid line  + circle markers
#     - Clean:  dashed line + square markers
#     - Noisy:  dotted line + triangle markers
#     - ACC/F1 shown in percent (0–100)
#     - No left/right padding on x-axis
#     """
#     # Expect exactly 2 JSON paths and labels
#     if len(json_paths) != 2:
#         raise ValueError("Provide exactly 2 JSON paths.")
#     if len(labels) != 2:
#         raise ValueError("Provide exactly 2 labels (one per JSON).")

#     # Line styles suitable for print / grayscale
#     LINE_STYLES = {
#         "test": {
#             "color": "tab:blue",
#             "linestyle": "-",
...
#     # Global alpha bounds (so both subplots line up) + remove x padding
#     global_alpha_min = min(min(a) for a, *_ in parsed)
#     global_alpha_max = max(max(a) for a, *_ in parsed)
...
#         h1, = ax.plot(
#             alphas,
#             [v * scale for v in test_vals],
#             label="Test Accuracy",
#             **LINE_STYLES["test"],
#         )
#         h2, = ax.plot(
#             alphas,
#             [v * scale for v in clean_vals],
#             label="Train Clean Accuracy",
#             **LINE_STYLES["clean"],
#         )
#         h3, = ax.plot(
#             alphas,
#             [v * scale for v in noisy_vals],
#             label="Train Noisy Accuracy",
#             **LINE_STYLES["noisy"],
#         )
...
def plot_acc_curves_from_jsons_1x2(
    json_paths,
    labels,
    metric_key="ACC",
    savepath=None,
    ...
    - Test:   blue, solid line  + circle markers
    - Clean:  green, dashed line + square markers
    - Noisy:  red, dotted line + triangle markers
    - ACC/F1 shown in percent (0–100)
    - No left/right padding on x-axis
    ...
    # Line styles (CVPR-friendly: color + linestyle + markers)
    LINE_STYLES = {
        "test": {
            "color": "tab:blue",
            "linestyle": "-",
    ...
    for ax, (title, series) in zip(axes, zip(labels, parsed)):
        ...
        h1, = ax.plot(
            alphas,
            [v * scale for v in test_vals],
            label=r"Accuracy on $\mathcal{D}_\text{test}$",
            **LINE_STYLES["test"],
        )
        h2, = ax.plot(
            alphas,
            [v * scale for v in clean_vals],
            label=r"Accuracy on $\mathcal{S}$",
            **LINE_STYLES["clean"],
        )
        h3, = ax.plot(
            alphas,
            [v * scale for v in noisy_vals],
            label=r"Accuracy on $\mathcal{C}$",
            **LINE_STYLES["noisy"],
        )
    ...
    # Shared Y label
    fig.supylabel(
        "Accuracy (%)" if scale_to_percent else metric_key,
