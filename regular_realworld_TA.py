...
def main():
    ...
    cfg_path = Path('configs/single_experiment/regular_realworld_TA') / f"{args.config}.yaml"

    if not cfg_path.exists(): raise RuntimeError('The specified config file does not exist.')
    with open(cfg_path, 'r') as file:
        cfg = yaml.full_load(file)

    outputs_dir = Path("outputs/single_experiment/regular_realworld_TA").absolute()
    results_dir = Path("results/single_experiment/regular_realworld_TA").absolute()
    results_dir.mkdir(exist_ok=True, parents=True)
