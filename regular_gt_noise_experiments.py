...
def finetune_models(outputs_dir: Path, results_dir: Path, cfg: dict, cfg_name:str):
    ...
    for discovery_rate in np.round(np.linspace(0.1, 1.0, 10), 1):
...
def apply_tv(outputs_dir: Path, results_dir: Path, cfg: dict, cfg_name:str):
    ...
    for discovery_rate in np.round(np.linspace(0.1, 1.0, 10), 1):
        if discovery_rate != 0.5:
            continue
        ft_expr_dir = outputs_dir / f"finetune_{discovery_rate}"
        n_weights = torch.load(
            ft_expr_dir.joinpath(f"weights/ft_weights.pth"),
            map_location='cpu'
        )
        noise_weights[f"Discovery Rate {discovery_rate}"] = n_weights
    ...
    for discovery_rate in np.round(np.linspace(0.1, 1.0, 10), 1):
        if discovery_rate != 0.5:
            continue
        results_dict[discovery_rate] = OrderedDict()
        for alpha in tqdm(np.linspace(-0.2, -10, 15)):
        
            model.load_state_dict(mix_weights, strict=False)
            task_vectors[f"Discovery Rate {discovery_rate}"].apply_to(model, scaling_coef=alpha, strict=False)
            ...
            results_dict[discovery_rate][alpha] = {'test_results': tv_test_results,  'train_results': tv_train_results}
        print(json.dumps(results_dict[discovery_rate], indent=4))
